...
# Translated and adopted from DeToX, Ringeval et al., 2024, https://doi.org/10.1093/bib/bbae094
#
...
def get_db(transcriptome_fasta: Path, *, mmseqs_path: str) -> Path:
    ...
    command = [
        ...
        db_path,  # Output file
        "--dbtype", "0",
        # Auto-detect if FASTA contains DNA/RNA or AA sequences (the latter is interesting in case of proteome given)
        "--shuffle", "1",  # Shuffle for higher resistance against statistical side effects
        "--createdb-mode", "0",
        # Copy data into DB. This is the best choice as it doesn't limit how the input file needs to look.
